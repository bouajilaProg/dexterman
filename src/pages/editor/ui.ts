...
const parseEnvStorage = (): EnvEntry[] => {
  try {
    const raw = window.localStorage.getItem(ENV_STORAGE_KEY);
    if (!raw) {
      return [{ key: "API_URL", value: " " }];
    }
    ...
    return entries.length ? entries : [{ key: "API_URL", value: " " }];
  } catch {
    return [{ key: "API_URL", value: " " }];
  }
};

const readEnvRows = (): EnvEntry[] => {
  const container = document.getElementById("env-rows");
  if (!(container instanceof HTMLElement)) {
    return [];
  }

  return Array.from(container.querySelectorAll("[data-env-row]"))
    .map((row) => {
      const key =
        (
      ...
      const value =
        (row.querySelector("[data-env-value]") as HTMLInputElement | null)
          ?.value ?? " ";
      return {
        key,
        value: value === "" ? " " : value,
...
const renderEnvRows = (entries: EnvEntry[]) => {
  const container = document.getElementById("env-rows");
  if (!(container instanceof HTMLElement)) {
    return;
  }
  ...
  container.innerHTML = rows
    .map(
      (entry) => `
    <div data-env-row class="grid grid-cols-5 gap-2 items-center">
      <input data-env-key class="col-span-2 w-full border border-border-subtle bg-bg-base px-2 py-1.5 text-xs text-text-normal outline-none focus:border-border-strong" placeholder="VAR_NAME" value="${entry.key.replaceAll('"', "&quot;")}">
      <input data-env-value class="col-span-2 w-full border border-border-subtle bg-bg-base px-2 py-1.5 text-xs text-text-normal outline-none focus:border-border-strong" placeholder="value" value="${entry.value.replaceAll('"', "&quot;")}">
      <button type="button" data-env-remove class="text-xs text-accent-danger border border-accent-danger/40 bg-accent-danger/10 px-2 py-1 hover:bg-accent-danger/20 transition-colors">Del</button>
    </div>
  `,
    )
    .join("");
...
const saveEnvRows = () => {
  const rows = readEnvRows();
  const vars = rows.reduce<Record<string, string>>((acc, item) => {
    acc[item.key] = item.value || " ";
    return acc;
  }, {});
  window.localStorage.setItem(ENV_STORAGE_KEY, JSON.stringify(vars));
};
...
const initEnvPanel = () => {
  const openButton = document.querySelector("[data-env-toggle]");
  const closeButton = document.querySelector("[data-env-close]");
  const addButton = document.querySelector("[data-env-add]");
  const saveButton = document.querySelector("[data-env-save]");
  const rowsContainer = document.getElementById("env-rows");
  ...
  addButton?.addEventListener("click", () => {
    const entries = readEnvRows();
    entries.push({ key: "", value: " " });
    renderEnvRows(entries);
  });
  ...
  rowsContainer?.addEventListener("click", (event) => {
    ...
    const removeButton = target?.closest("[data-env-remove]");
    if (!removeButton) {
      return;
    }

    const row = removeButton.closest("[data-env-row]");
    row?.remove();
    if (!rowsContainer.querySelector("[data-env-row]")) {
      renderEnvRows([{ key: "", value: " " }]);
    }
  });
};
...
const setUrlSelection = (selection: EditorSelection | null) => {
  ...
  if (selection?.folder && selection.api) {
    next.searchParams.set(
      "path",
      getSelectionPath(selection.folder, selection.api),
    );
...
const getActiveIdentity = (): ActiveIdentity | null => {
  const activeCard = document.querySelector(
    '[data-api-item][data-api-active="true"]',
  );
  if (!(activeCard instanceof HTMLElement)) {
    return null;
  }

  const folderAttr = activeCard
    .closest("[data-folder-dropzone]")
  ...
  for (const folder of data.folders ?? []) {
    ...
    for (const api of folder.apis ?? []) {
      ...
      if (apiNames.has(apiName)) {
        throw new Error(
          `API '${apiName}' already exists in folder '${folderName}'`,
        );
      }
...
const pickSelection = (
  ...
  if (preferred?.api && !preferred.folder) {
    const rootApi = (data.apis ?? []).find(
      (item) => item.name === preferred.api,
    );
    if (rootApi) {
  ...
  if (preferred?.folder) {
    const folder = (data.folders ?? []).find(
      (item) => item.name === preferred.folder,
    );
    if (folder) {
      if (preferred.api) {
        const api = (folder.apis ?? []).find(
          (item) => item.name === preferred.api,
        );
        if (api) {
...
const buildPayload = (): EditorData => {
  ...
  if (active.folder === null) {
    ...
    api.method = getTextValue(
      "[data-editor-api-method]",
      api.method || "GET",
    ).toUpperCase();
  ...
  activeFolder =
    folders.find((folder) => folder.name === active.folder) ?? activeFolder;
...
const moveApiInCachedData = (
  ...
  const sourceIndex = source.apis.findIndex(
    (api) => api.name === apiName && (!apiPath || api.path === apiPath),
  );
  if (sourceIndex < 0) {
    return null;
  }
 ...
 */
const autosaveFolderMove = async ({
  sourceFolder,
  targetFolder,
  apiName,
  apiPath,
  ...
  const moved = moveApiInCachedData(
    sourceFolder,
    targetFolder,
    apiName,
    apiPath,
  ...
  if (activeAfterMove?.folder && activeAfterMove.name) {
    ...
    next.searchParams.set(
      "path",
      getSelectionPath(activeAfterMove.folder, activeAfterMove.name),
    );
  } else {
    next.searchParams.delete("folder");
    next.searchParams.delete("api");
    next.searchParams.set(
      "path",
      getSelectionPath(targetFolder, moved.movedApi.name),
    );
  }
...
const newFolder = async () => {
  ...
  const suggestedName = buildUniqueName(
    currentFolders.map((folder) => folder.name),
    "folder",
  );
...
const renameFolder = async (folderName: string) => {
  ...
  const data = await ensureCachedData();
  const nextData = cloneEditorData(data);
  const target = (nextData.folders ?? []).find(
    (folder) => folder.name === trimmedFolderName,
  );
  if (!target) {
  ...
  const preferred =
    active?.folder === trimmedFolderName
...
const newRootApi = async () => {
  ...
  const suggestedName = buildUniqueName(
    nextData.apis.map((api) => api.name),
    "api",
  );
...
const deleteRootApi = async (apiName: string) => {
  ...
  const preferred =
    active && !(active.folder === null && active.name === trimmedApiName)
      ? active.folder
        ? { folder: active.folder, api: active.name }
        : { api: active.name }
      : fallbackRootApi
...
const newApi = async (folderName: string) => {
  ...
  const data = await ensureCachedData();
  const nextData = cloneEditorData(data);
  const targetFolder = (nextData.folders ?? []).find(
    (folder) => folder.name === trimmedFolderName,
  );
  if (!targetFolder) {
    throw new Error(`Folder '${trimmedFolderName}' was not found`);
  ...
  const suggestedName = buildUniqueName(
    targetFolder.apis.map((api) => api.name),
    "api",
  );
  const nextApiName = await getModalName(
    `API name for folder '${trimmedFolderName}'`,
    suggestedName,
  );
  if (nextApiName === null) {
    return;
  ...
  if (targetFolder.apis.some((api) => api.name === nextApiName)) {
    throw new Error(
      `API '${nextApiName}' already exists in folder '${trimmedFolderName}'`,
    );
  }

  targetFolder.apis.push(createDefaultApi(nextApiName));
  await persistDataAndNavigate(nextData, {
...
const deleteApi = async (folderName: string, apiName: string) => {
  ...
  targetFolder.apis = targetFolder.apis ?? [];
  const apiIndex = targetFolder.apis.findIndex(
    (api) => api.name === trimmedApiName,
  );
  if (apiIndex < 0) {
    throw new Error(
      `API '${trimmedApiName}' was not found in folder '${trimmedFolderName}'`,
    );
  }
  ...
  const sameFolderFallback =
    targetFolder.apis[apiIndex] ?? targetFolder.apis[apiIndex - 1];
  let preferred: EditorSelection;
  if (
    active &&
    !(active.folder === trimmedFolderName && active.name === trimmedApiName)
  ) {
    preferred = active.folder
      ? { folder: active.folder, api: active.name }
      : { api: active.name };
