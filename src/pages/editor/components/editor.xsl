...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- Reusable: single method option with selected detection -->
  ...
  <!-- Reusable: field type dropdown with selected detection -->
  ...
  <!-- Reusable: YES/NO required badge -->
  ...
  <!-- Reusable: move up/down + delete action buttons -->
  ...
  <!-- Main editor layout -->
  <xsl:template match="/">
    <xsl:variable name="api" select="/collection/group[1]/api[1]"/>

    <div>
      <!-- JS clone templates (plain HTML, no XSLT needed) -->
